...
void ecrire_image(Image I)
{
    ...
    for (y=1; y<(H+1); y++)
    {
        for (x=1; x<(L+1); x++)
        {
            ...
            if (curr == BLANC) printf(".");
            else printf("#") ;
            
            if (x ==(L)) printf("\n") ; //On verifie si on est au bout de la largeur de l'image
            //Si c'est le cas on retourne à la ligne
        }
    }
...
Image negatif_image(Image I)
{
     ...
     for (y=1; y<(H+1); y++)
    {
        ...
        {
            curr = get_pixel_image(I,x,y);
            if (curr == BLANC) set_pixel_image(negatif,x,y,NOIR);
            else set_pixel_image(negatif,x,y,BLANC) ;
