...
int main() {
    ...
    enfilerPoint(L,P0);
    enfilerPoint(L,P1);
    B = approx_bezier3(L->tete, L->queue);

    printf("P0:(%.0f, %.0f), P1:(%.0f, %.0f)\n", P0.x, P0.y, P1.x, P1.y);
    printf("B = [(%.2f, %.2f),(%.2f, %.2f),(%.2f, %.2f),(%.2f, %.2f)]\n", B.C0.x, B.C0.y, B.C1.x, B.C1.y, B.C2.x, B.C2.y, B.C3.x, B.C3.y);

    printf("(2P0 + P1)/3 = (%.2f, %.2f) ( = C1 )\n", (2*P0.x+P1.x)/3, (2*P0.y+P1.y)/3);
    printf("(P0 + 2P1)/3 = (%.2f, %.2f) ( = C2 )\n", (P0.x+2*P1.x)/3, (P0.y+2*P1.y)/3);
    ...
    P0 = creerPoint(rand()%20-10, rand()%20-10);
    P1 = creerPoint(rand()%20-10, rand()%20-10);
    P2 = creerPoint(rand()%20-10, rand()%20-10);
    enfilerPoint(L,P0);
    enfilerPoint(L,P1);
    enfilerPoint(L,P2);
    B = approx_bezier3(L->tete, L->queue);

    printf("P0:(%.0f, %.0f), P1:(%.0f, %.0f), P2:(%.0f, %.0f)\n", P0.x, P0.y, P1.x, P1.y, P2.x, P2.y);
    printf("B = [(%.2f, %.2f),(%.2f, %.2f),(%.2f, %.2f),(%.2f, %.2f)]\n", B.C0.x, B.C0.y, B.C1.x, B.C1.y, B.C2.x, B.C2.y, B.C3.x, B.C3.y);

    printf("(4P1 - P2)/3 = (%.2f, %.2f) ( = C1 )\n", (4*P1.x-P2.x)/3, (4*P1.y-P2.y)/3);
    printf("(4P1 - P0)/3 = (%.2f, %.2f) ( = C2 )\n", (4*P1.x-P0.x)/3, (4*P1.y-P0.y)/3);
    ...
    Q3 = creerPoint(rand()%20-10, rand()%20-10);
    printf("Q0:(%.0f, %.0f), Q1:(%.0f, %.0f), Q2:(%.0f, %.0f), Q3:(%.0f, %.0f)\n", Q0.x, Q0.y, Q1.x, Q1.y, Q2.x, Q2.y, Q3.x, Q3.y);
    B.C0 = Q0;
    B.C1 = Q1;
    B.C2 = Q2;
    B.C3 = Q3;
